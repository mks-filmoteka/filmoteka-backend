...
import static io.github.mksfilmoteka.catalog.util.TestUtil.testJwt;
...
class KeycloakRealmRoleConverterTest {
    ...
    void shouldReturnScopeAuthoritiesWhenRealmAccessIsMissing() {
        Jwt jwt = testJwt(Map.of("scope", "openid profile"));
        Collection<GrantedAuthority> result = converter.convert(jwt);
        assertThat(result)
                .extracting(GrantedAuthority::getAuthority)
    ...
    void shouldReturnScopeAuthoritiesWhenRolesClaimIsMissing() {
        Jwt jwt = testJwt(Map.of("scope", "openid", "realm_access", Map.of()));
        Collection<GrantedAuthority> result = converter.convert(jwt);
        assertThat(result)
                .extracting(GrantedAuthority::getAuthority)
